...
class SecurityPkiFixture(val pkiDir: Path) extends StrictLogging {
  ...
  def generate(networkAlias: String = "opensearch"): Unit = {
    ...
    runKeytool(
      ...
      "-keysize", "2048",
      "-validity", "1",
      "-keystore", pkiDir.resolve("root-ca.jks").toString,
      "-storepass", "changeit",
      "-keypass", "changeit",
      "-dname", "CN=root-ca,O=lenses,L=test,C=GB",
    ...
    runKeytool(
      "-importkeystore",
      "-srckeystore", pkiDir.resolve("connect-client.jks").toString,
      "-destkeystore", keystorePath.toString,
      "-srcstorepass", "changeit",
      "-deststorepass", "changeit",
      "-srcalias", "connect-client",
      "-destalias", "connect-client",
      "-noprompt",
    )
  ...
  private def generateSignedCert(
    ...
    runKeytool(
      ...
      "-keystore", pkiDir.resolve("root-ca.jks").toString,
      "-storepass", "changeit",
      "-infile", csrPath.toString,
      "-outfile", signedCert.toString,
      "-validity", "1",
      "-ext", s"SAN=$san",
      "-rfc",
    )
  ...
  private def exportKeytoolCert(keystoreFile: String, alias: String, outputPem: Path): Unit =
    runKeytool(
      "-exportcert",
      "-alias", alias,
      "-keystore", pkiDir.resolve(keystoreFile).toString,
      "-storepass", "changeit",
      "-file", outputPem.toString,
      "-rfc",
    )
  ...
  private def exportKeytoolKey(keystoreFile: String, alias: String, outputPem: Path): Unit = {
    ...
    runKeytool(
      "-importkeystore",
      "-srckeystore", pkiDir.resolve(keystoreFile).toString,
      "-destkeystore", p12.toString,
      "-srcstorepass", "changeit",
      "-deststorepass", "changeit",
      "-srcalias", alias,
      "-destalias", alias,
