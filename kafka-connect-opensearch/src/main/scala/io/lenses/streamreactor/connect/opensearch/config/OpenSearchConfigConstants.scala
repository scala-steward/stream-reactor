...
object OpenSearchConfigConstants {
  ...
  val JWT_TOKEN_FILE_DOC     = "Path to a file containing the JWT bearer token. Re-read at the configured interval. Mutually exclusive with connect.opensearch.security.jwt.token."
  ...
  val AWS_SIGNING_ENABLED_DOC      = "Enable AWS SigV4 request signing. When true, the transport switches from RestClientTransport to AwsSdk2Transport."
  val AWS_SIGNING_ENABLED_DEFAULT  = false

  val AWS_REGION_KEY      = s"$CONNECTOR_PREFIX.aws.region"
  val AWS_REGION_DOC      = "AWS region. Required when aws.signing.enabled=true. Validated against the SDK's known-region list at config time."
  val AWS_REGION_DEFAULT  = ""

  val AWS_SIGNING_SERVICE_KEY     = s"$CONNECTOR_PREFIX.aws.signing.service"
  val AWS_SIGNING_SERVICE_DOC     = "AWS service name for SigV4 signing. Must be one of {es, aoss}. 'es' for Managed OpenSearch, 'aoss' for OpenSearch Serverless."
  val AWS_SIGNING_SERVICE_DEFAULT = "es"

  val AWS_CREDENTIALS_PROVIDER_KEY     = s"$CONNECTOR_PREFIX.aws.credentials.provider"
  val AWS_CREDENTIALS_PROVIDER_DOC     = """Credentials provider type. DEFAULT or STATIC.
  ...
  val AWS_SESSION_TOKEN_DOC     = "AWS session token. Optional, for STS / assumed-role workflows. AWS session token configured via connect.opensearch.aws.session.token; this token does NOT auto-refresh and the connector will fail when it expires. Use connect.opensearch.aws.credentials.provider=DEFAULT for production deployments."
  ...
  val BULK_STRICT_ITEM_ERRORS_DOC     = """When true (default), any per-item bulk failure goes through ErrorPolicy.
