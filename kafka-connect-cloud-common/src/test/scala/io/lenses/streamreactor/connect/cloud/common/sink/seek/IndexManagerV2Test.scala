...
class IndexManagerV2Test
  ...
  // ── createNewIndexFileNoOverwrite: master-lock create race ─────────────────

  test(
    "open adopts the existing master lock when writeBlobToFile NoOverwrite loses a concurrent create race",
  ) {
    val tp              = Topic("topic1").withPartition(0)
    val bucketAndPrefix = CloudLocation("bucket", "prefix".some)
    ...
    when(bucketAndPrefixFn(any[TopicPartition])).thenReturn(Right(bucketAndPrefix))
    when(si.pathExists(anyString(), anyString())).thenReturn(Right(false))
    when(oldIndexManager.seekOffsetsForTopicPartition(tp)).thenReturn(Right(None))

    // Initial tryOpen returns FileNotFoundError (lock not yet written).
    // Re-read after NoOverwrite failure returns the winning task's lock.
    val notFound: Either[FileNotFoundError, ObjectWithETag[IndexFile]] =
      Left(FileNotFoundError(new Exception("not found"), "0.lock"))
    val existingLock: Either[Nothing, ObjectWithETag[IndexFile]] =
      Right(ObjectWithETag(IndexFile("other-task-uuid", Some(Offset(5)), None), "adopted-etag"))

    org.mockito.Mockito.doReturn(notFound, existingLock)
      .when(si).getBlobAsObject[IndexFile](anyString(), ArgumentMatchers.endsWith("0.lock"))(
        ArgumentMatchers.eq(indexFileDecoder),
      )
    ...
    try {
      val result = im.open(Set(tp))
      result.isRight shouldBe true
      // The adopted lock had committedOffset=Some(Offset(5))
      result.value.get(tp) shouldBe Some(Some(Offset(5)))
      // The adopted eTag must be cached so subsequent conditional writes fence correctly.
      im.topicPartitionToETags.get(tp) shouldBe Some("adopted-etag")
    } finally im.close()
  }

  test(
    "open returns FatalCloudSinkError when NoOverwrite fails and re-read also returns FileNotFoundError",
  ) {
    val tp              = Topic("topic1").withPartition(0)
    val bucketAndPrefix = CloudLocation("bucket", "prefix".some)
    ...
    // Both tryOpen calls (initial and re-read fallback) return FileNotFoundError.
    when(si.getBlobAsObject[IndexFile](anyString(), ArgumentMatchers.endsWith("0.lock"))(
      ArgumentMatchers.eq(indexFileDecoder),
