...
class JsonPayloadExtractorTest extends AnyFunSuite with Matchers with EitherValues with OptionValues {
  ...
  test("handle null values") {
    JsonPayloadExtractor.extractJsonNode(null, null) shouldBe Right(None)
  }

  test("handle string JSON with schema") {
    val result = JsonPayloadExtractor.extractJsonNode("""{"name": "test"}""", Schema.STRING_SCHEMA)
    result.value.value.get("name").asText() shouldBe "test"
  }

  test("primitive string that is not JSON is read as TextNode") {
    val result = JsonPayloadExtractor.extractJsonNode("invalid json", Schema.STRING_SCHEMA)
    result.value.value shouldBe a[TextNode]
    result.value.value.asText() shouldBe "invalid json"
  }

  test("handle byte array with schema") {
    val bytes  = """{"name": "test"}""".getBytes("UTF-8")
    val result = JsonPayloadExtractor.extractJsonNode(bytes, Schema.BYTES_SCHEMA)
    result.value.value.get("name").asText() shouldBe "test"
  }

  test("handle ByteBuffer with schema") {
    val byteBuffer = ByteBuffer.wrap("""{"name": "test"}""".getBytes("UTF-8"))
    val result     = JsonPayloadExtractor.extractJsonNode(byteBuffer, Schema.BYTES_SCHEMA)
    result.value.value.get("name").asText() shouldBe "test"
  }
  ...
  test("handle Struct with schema") {
    val schema = SchemaBuilder.struct()
      ...
      .build()
    val struct = new Struct(schema).put("name", "John").put("age", 30)
    val result = JsonPayloadExtractor.extractJsonNode(struct, schema)
    result.value.value.get("name").asText() shouldBe "John"
    result.value.value.get("age").asInt() shouldBe 30
  }
  ...
  test("handle Map without schema") {
    ...
    result.value.value.get("name").asText() shouldBe "test"
    result.value.value.get("value").asInt() shouldBe 42
  }
  ...
  test("handle nested Map without schema") {
    ...
    result.value.value.get("details").get("age").asInt() shouldBe 25
    result.value.value.get("details").get("city").asText() shouldBe "New York"
  }

  test("handle string JSON without schema (schema-less path)") {
    val result = JsonPayloadExtractor.extractJsonNode("""{"name": "test"}""", null)
    result.value.value.get("name").asText() shouldBe "test"
  }
  ...
  test("handle int value without schema (schema-less path)") {
    val result = JsonPayloadExtractor.extractJsonNode(42, null)
    result.value.value.asInt() shouldBe 42
  }

  test("returns Left when value type does not match STRING schema") {
  ...
  test("returns Left when value is not a Struct but schema is STRUCT") {
  ...
  test("returns Left when value is not bytes but schema is BYTES") {
