...
/**
 * Smoke-wiring test: verifies that JsonBulkWriter → KElasticBulkClient → KElasticClient.execute
...
class ElasticWriterWiringTest extends AnyWordSpec with Matchers with MockitoSugar with ArgumentMatchersSugar {
  ...
  "KElasticBulkClient wiring" should {
    "call KElasticClient.execute for each write batch" in {
      ...
      val bulkClient = new KElasticBulkClient(elasticClient, writeTimeoutMs = 5000)
