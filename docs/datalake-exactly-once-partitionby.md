...
The high-watermark is cleared in two paths. `close()` clears it for all partitions during a rebalance or task shutdown; because `open()` re-reads the master lock from storage for every newly assigned partition, the high-watermark is then re-initialized from the freshly read master lock offset on the first `preCommit` call after re-assignment. `cleanUp(tp)` clears it for a single partition when `CloudSinkTask.rollback` handles a `FatalCloudSinkError` from a failed `put()` -- the partition remains owned by this task instance, so the master lock in cloud storage is unchanged and the cached eTag stays valid. The high-watermark is re-seeded from the **preserved** master-lock seeked offset on the very next `preCommit` call for that partition (`getOrElseUpdate` re-reads `indexManager.getSeekedOffsetForTopicPartition`), so `globalSafeOffset` cannot regress below the durable master lock floor even though the in-memory watermark was momentarily cleared.
...
### In-place rollback after a fatal `put()` error (LC-2451 / ZD-2451)

During normal operation a `put()` call can surface a `FatalCloudSinkError` -- for example, when a transient GCS upload error is promoted to fatal by `PendingOperationsProcessors`. `CloudSinkTask.handleErrors` detects that `error.rollBack() == true` (the invariant of `FatalCloudSinkError`) and calls `rollback(topicPartitions)` → `WriterManager.cleanUp(tp)` for each affected partition. Unlike a rebalance or shutdown, **the partition is still assigned to this task instance** and the master lock file in cloud storage has not changed.

#### What `cleanUp` does in the rollback path

- Closes all writers for the partition and removes them from the `writers` map.
- Clears `safeOffsetHighWatermarks(tp)` so the next `preCommit` re-seeds the floor from the master lock.
- Calls `indexManager.evictAllGranularLocks(tp)` so a fresh writer for this partition reloads its dedup floor from storage rather than reading stale cached offsets.
- **Does NOT call `indexManager.clearTopicPartitionState(tp)`** -- this deliberately preserves `seekedOffsets` and `topicPartitionToETags` in `IndexManagerV2`.

#### Why preserving master-lock state is correct

The eTag stored in `topicPartitionToETags` was obtained the last time this task successfully wrote the master lock (or read it during `open()`). Because the partition is still owned by the same task instance and no other task has modified the master lock, the cached eTag still matches the live object in GCS. Clearing it (the pre-fix behaviour) left `IndexManagerV2.updateMasterLock` with no eTag to supply to the `If-Match` precondition, causing it to return `Left(FatalCloudSinkError("Master index not found", tp))`. `WriterManager.getOffsetAndMeta` catches that `Left`, logs it at ERROR level, and returns `None` -- which causes `preCommit` to emit no offset for the partition. Since Kafka Connect never received an updated consumer offset, the consumer group perceived the task as stuck and consumer lag grew without bound, even though record writes continued to land successfully in GCS.

#### Kafka Connect retry behaviour

After `handleErrors` calls `rollback` and throws `ConnectException`, the Connect framework retries `put()` with the same batch of records (controlled by `errors.retry.delay.max.ms` / `RetryErrorPolicy`). Critically, `open()` is **not** called between the failure and the retry -- there is no rebalance, no re-read of the master lock, and no rebuild of `topicPartitionToETags`. The retried `put()` creates new writers for the partition. On the subsequent `preCommit`:

1. `safeOffsetHighWatermarks` is empty for the partition (cleared by `cleanUp`), so `getOrElseUpdate` calls `indexManager.getSeekedOffsetForTopicPartition(tp)` -- which reads from the **preserved** `seekedOffsets` -- to set the floor.
2. `updateMasterLock` finds the **preserved** eTag in `topicPartitionToETags` and proceeds with the conditional write normally.
3. Consumer offset advance resumes.

#### Relationship to zombie-fencing

The "no eTag refresh on failure" rule in `updateMasterLock` prevents a zombie task from re-reading the current eTag from storage and stealing the new task's fencing token after an eTag mismatch. The `cleanUp` scenario is different in kind: no write was attempted at all (the error occurred before `updateMasterLock` was reached), so the cached eTag is not stale -- it is still the correct fencing token for this task. Preserving it is therefore safe and required, not in tension with the fencing rule. On the fixed code, a cache miss in `updateMasterLock` can only be reached through `clearTopicPartitionState`, which is now called exclusively by `IndexManagerV2.open()` (during rebalance stale-partition pruning or its parTraverse failure rollback) -- both paths that are immediately followed by a fresh `open()` read of the master lock.

### Master lock fallback for new and GC'd partition keys

When `createWriter` initializes a PARTITIONBY writer, it first attempts to load the writer's granular lock via `getSeekedOffsetForPartitionKey`. If the granular lock does not exist in storage (returns `Right(None)`), the writer falls back to the master lock offset via `getSeekedOffsetForTopicPartition`. This master lock offset becomes the writer's `lastSeekedOffset`, which `shouldSkip` uses as a deduplication floor: any offset at or below `lastSeekedOffset` is treated as already committed and skipped.

**Why it is safe.** When `globalSafeOffset == 0`, `updateMasterLock` stores `None` (not `Some(Offset(0))`), so `getSeekedOffsetForTopicPartition` returns `None` and the fallback produces `None.orElse(None) = None` -- no false skip of offset 0. When `globalSafeOffset > 0` (e.g. 105), the master lock stores `Some(Offset(104))`, so the fallback provides `Some(Offset(104))` as the deduplication floor. Since the Kafka consumer is at offset 105+, new records are above 104 and `shouldSkip` returns false. On crash recovery, offset 104 routes to its original writer (deterministic keys) whose granular lock is preserved by the GC threshold. The fallback only participates when no granular lock exists, and in that case the master lock offset is the best available deduplication floor.

**Interaction with GC.** The GC threshold (`< globalSafeOffset - 1`) preserves the granular lock at `masterOffset`. The fallback provides a second layer of defense: if the GC threshold were accidentally loosened in a future change (e.g. `<` changed to `<=`), the granular lock at `masterOffset` would be deleted, but the master lock offset would still prevent duplication of the replayed record at `masterOffset`.

**Interaction with manual consumer rewind.** Without the fallback, rewinding the consumer below `globalSafeOffset` causes full duplication of records whose granular locks were GC'd -- the writer starts with `lastSeekedOffset = None` and re-writes every record. With the fallback, `shouldSkip` uses the master lock offset as a floor, correctly skipping records at or below the master lock's `committedOffset`. Duplication is limited to records above the master lock offset (which may have been committed by individual writers but not yet reflected in a `globalSafeOffset` update).

**Interaction with the orphan sweep.** The sweep uses the same threshold as GC (`committedOffset < masterOffset`) and the same cache-gated drain. If a sweep deletes a granular lock and a new writer is subsequently created for that partition key, `ensureGranularLock` creates an empty lock and `getSeekedOffsetForPartitionKey` returns `None`. The fallback provides the master lock offset, preventing the writer from re-processing already-committed records.
...
After a successful master lock update, `cleanUpObsoleteLocks` identifies obsolete granular locks -- those whose `committedOffset` is strictly below `masterOffset` (`globalSafeOffset - 1`), **or** whose `committedOffset` is unset (empty locks created by `ensureGranularLock` but never committed to) -- and that do not belong to an active writer. The threshold preserves the granular lock at `masterOffset` itself, which is needed by `shouldSkip` to deduplicate the one-record overlap on replay. Empty locks carry no durable data, so deleting them cannot affect the lock at `masterOffset`; without this branch, abandoned eTag-only entries would never be evicted from the in-memory cache once their writer is gone, and would never be enqueued for cloud-storage deletion. For each obsolete entry it:
...
4. **Offset comparison**: The lock file is read and its `committedOffset` is compared against the master lock offset (`seekedOffsets.get(tp)`). Only lock files with `committedOffset` strictly below `masterLockOffset` are enqueued as orphans. The strict-less-than preserves the granular lock at `masterOffset` itself, which is needed by `shouldSkip` to deduplicate the one-record overlap on replay. This matches the threshold used by `cleanUpObsoleteLocks` (`committedOffset < globalSafeOffset - 1`). In addition, lock files with **no `committedOffset` and no `PendingState`** (empty locks created by `ensureGranularLock` but never committed to) are also enqueued: they carry no durable data, the recency filter has already proved they are not freshly created, and the cache filter has already proved no active writer is using them. Without this branch, empty locks would accumulate indefinitely in cloud storage under high-cardinality `PARTITIONBY` with churning keys.

Lock files with no `committedOffset` but a `PendingState` are conservatively **not** enqueued. The commit protocol never produces this shape -- `PendingState` is only written alongside a `Some(committedOffset)` -- so this branch is a defensive guard against a hypothetical state where pending operations would otherwise be silently lost. Partitions without a master lock offset in `seekedOffsets` (e.g. brand-new partitions that have never had a commit) are also skipped -- there is no safe baseline to compare against.
...
1. **No re-read on write failure** (`updateMasterLock` / `updateForPartitionKey`): After a conditional write to cloud storage fails (network error, eTag mismatch), the cached eTag is deliberately not refreshed. A stale eTag means the task cannot fence on the current state, which is the correct behavior for a zombie task that should not advance.

2. **Cache miss is a fail-loud signal** (`updateMasterLock` returning `FatalCloudSinkError("Master index not found")`): Before the LC-2451 fix, `WriterManager.cleanUp` could clear the master eTag from the cache while the partition was still owned by the task and no write failure had occurred. This is a different scenario: no write was attempted, the cached eTag was simply discarded, and re-reading from storage would have been safe. The fix ensures this scenario cannot arise: `clearTopicPartitionState` is now called only by `IndexManagerV2.open()`, which immediately follows it with a fresh master lock read. A cache miss in `updateMasterLock` therefore indicates a code path that bypassed `open()` -- a programming error, not a zombie-fencing scenario -- and failing loudly is the appropriate response.
...
The granular lock cache is an unbounded `java.util.concurrent.ConcurrentHashMap`. It does **not** perform automatic LRU eviction. Instead, entries are added when writers are created and removed by `cleanUpObsoleteLocks` (GC enqueue phase), `evictAllGranularLocks` (shutdown / rebalance / fatal-error rollback), or explicit `evictGranularLock` calls. `ConcurrentHashMap` is required because the background GC thread reads the cache (`containsKey`) to check whether a scheduled-for-deletion key has been reclaimed by a new writer (see "Drain phase" under "Garbage collection"). All mutating access occurs on the single Kafka Connect task thread.
...
- `WriterManager.cleanUp(topicPartition)`: evicts all granular lock entries for the partition via `evictAllGranularLocks`, but deliberately does **not** call `clearTopicPartitionState` -- the partition is still owned by this task (this is the `CloudSinkTask.rollback` path after a `FatalCloudSinkError`, not a rebalance), so the master-lock eTag and `seekedOffsets` entry in `IndexManagerV2` must be preserved for the next `preCommit` cycle.
